...
AlphaDesk is an AI-assisted quantitative research platform for Indian markets. This repository contains the **Phase 4 research foundation**: point-in-time market data, a versioned technical feature engine, the historical Market Scanner, revision-aware corporate actions, and a deterministic versioned strategy setup engine. It does not backtest, rank securities, generate recommendations, connect to brokers, or place real or paper orders.
...
```text
frontend → FastAPI routes → strategy/scanner/technical services → repositories → SQLAlchemy → PostgreSQL
...
The six implemented workspaces are Dashboard, Securities, Security Detail (Overview plus Technicals), Market Scanner, Strategy Research, and Data Health. Strategy Research renders metadata-driven parameter controls and explainable matched/not-matched condition results. All values come from AlphaDesk APIs; the UI does not fabricate live prices, signals, rankings, or recommendations.
...
Tests cover the Phase 1 controls plus technical formulas and warm-ups, adjusted split/bonus continuity, point-in-time availability/revisions, catalogs, versions, batch/latest equivalence, bounded query shape, scanner validation, and the Phase 4 strategy registry, price structure, rules, parameters, fingerprints, historical membership, EOD timing, API behavior, deterministic ordering, and future-data invariance.
...
- Scanner expressions remain a flat AND list and strategy rules are immutable code definitions. There is no OR/grouping, ranking, saved evaluation, authentication, streaming feed, intraday strategy behavior, background worker, cache, backtester, performance statistic, ML, portfolio logic, broker, or order model.
- PostgreSQL is the production target; SQLite is used only by isolated tests and migration smoke checks.

## Architecture decision after Phase 4

The strategy engine reuses the dynamic projected batch path and passed ≤2 s at 50 members, ≤5 s at 200, and ≤10 s at 500 while returning all members and six condition details. No strategy persistence, snapshot, cache, or feature store is justified by this workload. Phase 5 was not started automatically.

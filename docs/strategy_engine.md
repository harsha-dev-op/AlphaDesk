...
Phase 7 consumes multiple registered strategy definitions through the same condition evaluator and one shared technical-feature batch. It canonicalizes component order and composes typed outcomes under a separate `CONSENSUS_N_OF_M` v1 registry; it does not modify strategy rules, defaults, fingerprints, or execution semantics. Saved experiments store registry keys, normalized parameters, and compact provenance rather than copying executable rules. See [research composition specification](research_composition.md).

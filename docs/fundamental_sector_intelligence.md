...
## Phase 13D scalable non-financial ingestion

The adapter is reusable across standard non-financial NSE Ind AS issuers. Company identity is no longer hardcoded: listing symbol, XBRL symbol/ISIN, and the official NSE EQ security master must agree. Financial-sector symbols and unreviewed taxonomy families are rejected rather than coerced into the corporate concept model.

Accepted taxonomy families remain explicit and versioned: `SEBI_IN_CAPMKT_2025` and `SEBI_IN_CAPMKT_2026`. A numeric fact in another taxonomy namespace rejects that filing and records the namespace in inspection output when other valid filings remain. Raw QName, namespace, context, unit, decimals, source locator, checksum, scope, audit state, and official availability are retained unchanged.

The bounded acquisition helper operates only from official links already present in an operator-provided NSE listing CSV. Its deterministic manifest and checksum rules provide resumability without overwriting raw files. It does not discover private endpoints, evade access controls, or infer filenames. Data Health includes mapped/unmapped fact counts, current NIFTY 200 issuer coverage, consolidated/standalone counts, and supported/unsupported taxonomy-family observations while overall fundamentals remain `PARTIAL`.

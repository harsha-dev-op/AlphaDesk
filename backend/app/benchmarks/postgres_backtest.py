...
TARGET_SECONDS = {50: 5, 200: 15, 500: 30}


def _request(size: int) -> BacktestRunRequest:
...
def _instrumented_run(request: BacktestRunRequest) -> dict[str, object]:
    query_count = 0
    ...
    try:
        with SessionLocal() as session:
            ...
            response = BacktestService(session).run(request)
    ...
    return {
        ...
        "trade_count": response.total_trade_count,
...
def _api_timing(request: BacktestRunRequest) -> dict[str, object]:
    with TestClient(app) as client:
        started = time.perf_counter()
        response = client.post("/api/v1/backtests/run", json=request.model_dump(mode="json"))
        elapsed_ms = (time.perf_counter() - started) * 1_000
    if response.status_code != 200:
        raise RuntimeError(f"Backtest benchmark API failed with HTTP {response.status_code}")
    payload = response.json()
    return {
        "api_ms": round(elapsed_ms, 3),
        "setup_count": payload["setup_count"],
        "trade_count": payload["total_trade_count"],
...
def run_benchmarks(runs: int, sizes: tuple[int, ...] = PROFILE_SIZES) -> dict[str, object]:
    ...
    if unsupported:
        raise ValueError(f"Unsupported backtest benchmark sizes: {sorted(unsupported)}")
    with SessionLocal() as session:
    ...
    for size in sizes:
        ...
        if len(fingerprints) != 1:
            raise RuntimeError(f"Non-deterministic backtest fingerprint at size {size}")
        expected_queries = 4 + 2 * math.ceil(size / 50)
        if any(run["query_count"] != expected_queries for run in individual):
            raise RuntimeError(f"Unexpected backtest query count at size {size}")
        api = _api_timing(request)
        if api["run_fingerprint"] not in fingerprints:
            raise RuntimeError(f"API and direct backtest fingerprints differ at size {size}")
        warm = individual[1:]
    ...
    return {
        ...
        "backtest": "BREAKOUT_20D v1 / NEXT_OPEN_FIXED_HOLD v1 / ADJUSTED",
        "cases": cases,
        "measurement_notes": [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Read-only PostgreSQL historical backtest scale benchmark")
